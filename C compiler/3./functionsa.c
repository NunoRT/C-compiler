...
token* create_token(char *nome,int linha,int coluna){
	...
	token* no=(token*)malloc(sizeof(struct Token));
	no->nome=strdup(nome);
	no->linha=linha;
	no->coluna=coluna;
	return no;
...
//funcao extra da meta 3
void add_sem(No* node,char*insere){
	node->sem=strdup(insere);
...
void print_tree(No* n, int nivel) {
	...
	if(n->sem!=NULL && n->valor!=NULL) printf("%s(%s) - %s\n", n->tipo, n->valor, n->sem);
	else if(n->sem!=NULL && n->valor==NULL) printf("%s - %s\n", n->tipo, n->sem);
	else if(n->valor == NULL) printf("%s\n", n->tipo);
